...
const HotelOrderMenuItem = ({ item }) => {
    ...
    const getMenuItem = async () => {
      try {
        const result = await axios.get(
          `http://localhost:8080/hotel/menu?id=${item}`,
          {
            headers: {
  ...
  return (
    <Card style={{ border: "2px solid #ccc", borderRadius: "10px" }}>
      <Card.Body>
        <Card.Title>
          <strong>Item Name:</strong> {menuItem.menuItemName}
        </Card.Title>
        <Card.Text>
          <strong>Item Price:</strong> {menuItem.menuItemPrice}
        </Card.Text>
        <Card.Text>
          <strong>Item Rating:</strong> {menuItem.menuItemRating}
        </Card.Text>
        <Card.Text>
          <strong>Item is veg:</strong>
          {menuItem.isVeg}
        </Card.Text>
      </Card.Body>
    </Card>
