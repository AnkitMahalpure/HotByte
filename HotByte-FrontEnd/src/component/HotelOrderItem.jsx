  ...
  const handleItemClick = async (item) => {
    ...
    try {
      const result = await axios.put(
        `http://localhost:8080/hotel/order/update?statusId=${statusID}&orderId=${order.orderID}`,
        {},
        {
          headers: {
        ...
        <div className="col-5">
          <Card style={{ border: "2px solid #ccc", borderRadius: "10px" }}>
            <Card.Body>
              ...
              <Card.Text>
                <strong>UserID:</strong> {{ ...order.userID }.userId}
              </Card.Text>
              <Card.Text>
                <strong>Total:</strong> {order.amount}
              </Card.Text>
              <Card.Text>
                <strong>OrderTime:</strong> {order.orderTime}
              </Card.Text>
              <Card.Text>
                <strong>DeliveryTime:</strong> {order.deliveryTime}
              </Card.Text>
              <Card.Text>
                <strong>OrderStatus:</strong> {order.orderStatus}
              </Card.Text>
              <Card.Text>
